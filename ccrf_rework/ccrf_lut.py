...
def sigmaF1(comparasum):
    ...
    #    sum_col_tmp = 0
    #print("time taken for old =",time.time()-init_time)
    #init_time_2 = time.time()
    for i in range(257):
        #print(np.sum(comparasum[:][0:i],axis=0,keepdims=True))
        cdf_col[i-1][:] = np.sum(comparasum[:][0:i],axis = 0)
    #print("time taken for new =",time.time()-init_time_2)
    #try:
    #    assert (cdf_col==cdf_col_2).all()
    #except:
    #    print(cdf_col_2)
    #    print(cdf_col)
    #    sys.exit(0)
...
def sigmaF2(comparasum):
    ...
    for i in range(1,257):
        #print(np.sum(comparasum[0:i][:],axis=1,keepdims=True))
        cdf_row_2[:][i-1] = np.sum(comparasum[0:i][:],axis = 1,keepdims=True)
    #print("time taken for new =",time.time()-init_time_2)
    try:
        assert (cdf_row==cdf_row_2).all()
    except:
        print(cdf_row_2)
        print(cdf_row)
        sys.exit(0)
...
for index in range(1,2):
    ...
    compsum_B = np.loadtxt('compsum_B_{}_norm.txt'.format(k**index))
    ...
    compsum_G = np.loadtxt('compsum_G_{}_norm.txt'.format(k**index))
    ...
    compsum_R = np.loadtxt('compsum_R_{}_norm.txt'.format(k**index))
